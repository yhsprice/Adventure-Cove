const holes = [
  ...
  advanceHole();
    ...
    function applyEditableAdjustment(type) {
      ...
      askAnotherEditableAdjustment(type);
    }

    function askAnotherEditableAdjustment(type) {
    ...
    function generateHighlights(results) {
      ...
      results.forEach(p => {
        if (p.stats.worstHole) {
          const locationJoke = p.stats.worstHole === 18
            ? "The waterfall refuses to comment."
            : "The Cove crew has marked it as a suspicious splash-free disaster."
          highlights.push(`🌊 Hole ${p.stats.worstHole} gave ${p.name} some trouble. ${locationJoke}`);
        }
      });
